...
import { ComponentProps, createContext, PropsWithChildren, ReactNode, useContext, useState } from 'react'
import toast, { resolveValue, Toast, ToastOptions } from 'react-hot-toast'
...
function createAlertToast(type: AlertType, message: AlertToastMessage, options?: ToastOptions) {
  if (typeof message === 'string') {
    return toast.custom((t) => <AlertToast type={type} t={t} title={message} />, options)
  } else {
    return toast.custom(
      (t) => (
        <AlertToast type={type} t={t} title={message.title} description={message.description} action={message.action} />
      ),
      options,
    )
  }
...
const successAlertToast = (message: AlertToastMessage, options?: ToastOptions) =>
  createAlertToast('success', message, { duration: 2000, ...options })
const warningAlertToast = (message: AlertToastMessage, options?: ToastOptions) =>
  createAlertToast('warning', message, { duration: 4000, ...options })
const loadingAlertToast = (message: AlertToastMessage, options?: ToastOptions) =>
...
const alertBarStyles = cva('w-2 absolute left-0 inset-y-0 rounded-l-lg transition-colors', {
  variants: {
    type: {
      info: 'bg-system-info',
...
export const AlertContent = ({ className, children, ...rest }: ComponentProps<'div'>) => {
  const { type } = useAlertContext()
  return (
    <div role="alert" className={cn(alertContentStyles({ type, className }))} {...rest}>
...
export const AlertHeader = ({ children, onClose }: AlertHeaderProps) => {
  return (
    <div className="flex flex-1 justify-between items-center gap-4">
      <p className="text-body-medium-prominent lg:text-body-large-prominent text-text-on-surface">{children}</p>
      {onClose && (
        <Button onClick={onClose} variant="ghost" color="black" size="sm" className="p-0">
