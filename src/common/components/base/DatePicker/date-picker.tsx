...
function extractCalendarProps<TProps extends CommonCalendarProps>(props: TProps) {
  const { disabledDate, timePicker: _, disabledFuture, lastNMonth, canUnselect, ...rest } = props
  const calendarProps = { disabledDate, disabledFuture, lastNMonth, canUnselect }
  return { calendarProps, rest }
}

export const DatePicker = React.forwardRef<React.ElementRef<typeof PopoverTrigger>, DatePickerProps>(
...
export const DateTimePicker = React.forwardRef<React.ElementRef<typeof PopoverTrigger>, DatePickerProps>(
  ({ value, onChange, ...props }, ref) => {
    const { calendarProps, rest } = extractCalendarProps(props)
    return (
      <DatePickerProvider value={value} onChange={onChange}>
...
interface DateRangePickerProps
  extends DateRangePickerContextValue,
    CommonCalendarProps,
    Omit<DatePickerTriggerProps, 'value' | 'onChange'> {}
export const DateRangePicker = React.forwardRef<React.ElementRef<typeof PopoverTrigger>, DateRangePickerProps>(
  ({ value, onChange, ...props }, ref) => {
    const { calendarProps, rest } = extractCalendarProps(props)
    return (
...
const DatePickerContext = React.createContext(null as null | DatePickerContextValue)
const useDatePickerContext = () => {
  const context = React.useContext(DatePickerContext)
  if (context === null) {
    throw new Error('useDatePickerContext must be inside of DatePickerContext.Provider')
  }
  return context
}
...
export const DatePickerProvider = ({ children, ...props }: DatePickerProviderProps) => {
  return (
    <DatePickerContext.Provider value={{ value: props.value, onChange: props.onChange }}>
...
const DateRangePickerContext = React.createContext(null as null | DateRangePickerContextValue)
const useDateRangePickerContext = () => {
  const context = React.useContext(DateRangePickerContext)
  if (context === null) {
    throw new Error('useDateRangePickerContext must be inside of DateRangePickerContext.Provider')
  }
  return context
}
...
export const DateRangePickerProvider = ({ children, ...props }: DateRangePickerProviderProps) => {
  return (
...
export const DateRangePickerValue = ({ format = 'D MMMM BBBB', placeholder = 'select date' }: DatePickerValueProps) => {
...
interface TriggerProps extends React.ComponentPropsWithoutRef<typeof PopoverTrigger> {}
const Trigger = React.forwardRef<React.ElementRef<typeof PopoverTrigger>, TriggerProps>(
    ...
    return (
      <PopoverTrigger
        ...
        className={cn(
          ...
          'gap-2 data-[placeholder=true]:text-text-placeholder disabled:data-[placeholder=true]:text-text-disabled text-[16px]',
          className,
        )}
        {...props}
      >
...
export const DatePickerTrigger = React.forwardRef<React.ElementRef<typeof PopoverTrigger>, DatePickerTriggerProps>(
  ({ className, format, placeholder, ...props }, ref) => {
    const { value } = useDatePickerContext()
    return (
      <Trigger ref={ref} {...props} data-placeholder={!value} className={clsx('justify-between', className)}>
        <DatePickerValue format={format} placeholder={placeholder} />
      </Trigger>
    )
  },
)
export const DateRangePickerTrigger = React.forwardRef<React.ElementRef<typeof PopoverTrigger>, DatePickerTriggerProps>(
...
export const DateTimePickerTrigger = React.forwardRef<React.ElementRef<typeof PopoverTrigger>, DatePickerTriggerProps>(
...
export interface DatePickerCalendarProps extends Omit<DayPickerSingleProps, 'mode' | 'disabled'>, CommonCalendarProps {}
...
export const DatePickerCalendar = (props: DatePickerCalendarProps) => {
  const { timePicker, disabledFuture, lastNMonth, canUnselect, ...rest } = props
...
export const DateRangePickerCalendar = (props: DateRangePickerCalendarProps) => {
  const { timePicker, disabledFuture, lastNMonth, ...rest } = props
  const { value: dateRange, onChange: setDateRange } = useDateRangePickerContext()
    ...
    <DatePickerCalendar
      ...
      timePicker={
        <div className="p-3 border-t border-border">
          <TimePicker
            setDate={(_date) => {
              if (disabledDate instanceof Boolean && disabledDate) return true
              if (disabledDate instanceof Function && _date && disabledDate(_date)) return true
...
export const DateTimeRangePickerCalendar = (props: DateRangePickerCalendarProps) => {
  const disabledDate = props.disabledDate
  const { value: dateRange, onChange: setDateRange } = useDateRangePickerContext()
  return (
    <DateRangePickerCalendar
      ...
      numberOfMonths={2}
      timePicker={
        <div className="p-3 flex justify-around items-center border-t border-border">
          <TimePicker
            setDate={(_date) => {
              if (disabledDate instanceof Boolean && disabledDate) return
              if (disabledDate instanceof Function && _date && disabledDate(_date)) return
