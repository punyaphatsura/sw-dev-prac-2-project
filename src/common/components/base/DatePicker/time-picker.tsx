  ...
  (
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key >= '0' && e.key <= '9') {
        ...
        const tempDate = new Date(date)
        const result = setDate(setDateByType(tempDate, newValue, picker))
    ...
    return (
      <input
        ...
        name={name || picker}
        className={cn(
          'w-[24px] text-center font-sans text-base tabular-nums caret-transparent focus:text-text-on-surface outline-primary [&::-webkit-inner-spin-button]:appearance-none',
          className,
        )}
...
export function TimePicker({ date, disabled, setDate }: TimePickerProps) {
  ...
  return (
    <div className="focus-within:border-border-primary w-fit m-auto flex gap-2 text-center items-center border p-2 px-4 rounded-lg">
